...
Track which users are currently online (lobby vs in-game) using Phoenix Presence for real-time visibility and auto-cleanup on disconnect.
...
- **THEN** their presence is tracked in the `"user"` topic with metadata `%{location: "lobby"}`
...
- **THEN** their presence is tracked in the `"user"` topic with metadata `%{location: "game", game_id: game_id, mark: mark}`
