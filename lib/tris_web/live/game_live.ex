defmodule TrisWeb.GameLive do
  ...
  def handle_params(%{"id" => game_id} = params, _uri, socket) do
    game_exists? = Registry.lookup(Tris.GameRegistry, game_id) != []
    ...
    socket =
      if game_exists? do
        ...
        socket =
          ...
          |> assign(:my_name, game_state.names[player_mark])
          |> assign(:opponent_name, game_state.names[opponent_mark])
          |> assign(:result, nil)
        ...
        <%= cond do %>
          <% @game_not_found -> %>
            <div class="text-center py-16">
              <div class="text-4xl font-bold mb-4">Game not found</div>
              <p class="text-base-content/70 mb-6">
                This game does not exist or has already ended.
              </p>
              <p class="text-sm text-base-content/50">Redirecting to lobby...</p>
            </div>
          <% @game_state -> %>
